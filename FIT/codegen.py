...
class CodeGenerator:
    ...
    def generate_header(self):
        ...
        cw.new_line()
        cw.write(f'# Generated by {self.__class__.__name__} in {Path(__file__).name} based on profile version {self.profile.version.version_str()}')
...
class TypeCodeGenerator(CodeGenerator):
    ...
    def __init__(self, profile: Profile, code_writer: CodeWriter = None, unit_synonyms: Dict[str, str] = None, invalid_value_identifiers: Dict[str, str] = None, free_range_types: Iterable[str] = None):
...
class MessageCodeGenerator(CodeGenerator):
    ...
    def __init__(self, profile: Profile, code_writer: CodeWriter = None, unit_synonyms: Dict[str, str] = None):
        super().__init__(profile, code_writer)

        if unit_synonyms:
            self.unit_synonyms = unit_synonyms
        else:
            self.unit_synonyms = MessageCodeGenerator.DEFAULT_UNIT_SYNONYMS
    ...
    def generate_units(self):
        ...
        cw.unindent()
        cw.new_line(2)
        cw.write('def ufs(unit: str) -> Unit:')
        cw.indent()
        cw.write('return Unit.from_string(unit)')
        cw.unindent()
